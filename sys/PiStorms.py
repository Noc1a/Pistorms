...
class PiStormsMotor():
    ...
    def _ramp_to(self, speed, acceleration, set_fn, callback=None):
        current = self.psmotor.getSpeed()
        #print(f"Ramping from {current} to {speed} with acceleration {acceleration}")
        step = 1 if speed > current else -1
        delay = 1.0 / abs(acceleration)

        while current != speed and self.running:
            current = min(speed, current + step) if step > 0 else max(speed, current + step)
            #print(f"Setting speed to {current}")
            set_fn(current)
            time.sleep(delay)
        if callback:
            callback()

    def _setSpeed(self, speed, acceleration, blocking, set_fn, callback=None):
        ...
        ramp = lambda: self._ramp_to(speed, acceleration, set_fn, callback)
    ...
    #  @endcode
    def setSpeed(self, speed, acceleration=None, blocking=True, callback=None):
            self._setSpeed(speed, acceleration, blocking, self.psmotor.setSpeed, callback)
    ...
    #  @param acceleration The acceleration in percent per second (optional). If not provided, the speed will change immediately.
    #  @param blocking If True, the function will block until the target speed is reached. If False, the speed will change in the background and the function will return immediately. (default: True)
    #  @remark
    #  To use this function in your program:
    #  @code
    #  from PiStorms import PiStorms
    ...
    #  @endcode
    def setSpeedSync(self, speed, acceleration=None, blocking=True, callback=None):
            self._setSpeed(speed, acceleration, blocking, self.psmotor.setSpeedSync, callback)

    def setSpeedSyncOpposite(self, speed, acceleration=None, blocking=True, callback=None):
        self._setSpeed(speed, acceleration, blocking, self.psmotor.setSpeedSyncOpposite, callback)
    ...
    #  psm.BAM1.brakeSync()
    #  @endcode
    def brakeSync(self, acceleration=None, blocking=True):
        self.running = False
        if acceleration is None:
            self.psmotor.brakeSync()
        else:
            self.setSpeedSync(0, acceleration, blocking, self.psmotor.brakeSync)
    ...
    #  psm.BAM1.brake()
    #  @endcode
    def brake(self, acceleration=None, blocking=True):
        self.running = False
        if acceleration is None:
            self.psmotor.brake()
        else:
            self.setSpeed(0, acceleration, blocking, self.psmotor.brake)
