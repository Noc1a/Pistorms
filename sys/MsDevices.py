...
class AbsoluteIMU(mindsensors_i2c):
    ...
    def get_acc_mag_gyr(self):
        """
        Retrieves and converts IMU sensor data to the North-East-Down (NED) coordinate frame.
        This method reads raw sensor data from the Absolute IMU, converts the gyroscope, accelerometer, 
        and magnetometer values to standard SI units, and arranges them according to the NED convention:
        - Accelerometer: meters per second squared (m/s²)
        - Gyroscope: degrees per second (deg/s)
        - Magnetometer: nanoTesla (nT)
        
        Returns:
            tuple: A tuple containing three numpy arrays:
                - acc (list): Accelerometer values (m/s²) in NED order [North, East, Down].
                - gyr (list): Gyroscope values (deg/s) in NED order [North, East, Down].
                - mag (list): Magnetometer values (nT) in NED order [North, East, Down].
        """
        ...
        #raw = self.readArray(self.ACCEL_X, 20)
        result = struct.unpack("<10h", bytes(raw))
        return self._convert_acc(result[0:3]), self._convert_mag(result[4:7]), self._convert_gyro(result[7:10])
    ...
    def get_all_values(self):
        ...
        Returns:
            tuple: A tuple containing three numpy arrays:
                - tilt (tuple): Tilt values along x, y, z axes.
                - compass (int): Compass heading in degrees.
                - gyr (list): Gyroscope values (deg/s) in NED order [North, East, Down].
                - acc (list): Accelerometer values (m/s²) in NED order [North, East, Down].
                - mag (list): Magnetometer values (nT) in NED order [North, East, Down].
        """ 
        ...
        #raw = self.readArray(self.ACCEL_X, 23)
    ...
    def _convert_gyro(self, raw_gyro):
        """Converts raw gyroscope values to degres /secondes (deg/s) using the current sensitivity setting."""
        gyro = [0.0, 0.0, 0.0]
        for i in range(3):
    ...
    def _convert_acc(self, raw_acc):
        """Converts raw accelerometer values to meters per second squared (m/s²)."""
        acc = [0.0, 0.0, 0.0]
        for i in range(3):
    ...
    def _convert_mag(self, raw_mag):
        """Converts raw magnetometer values to nanoTesla (nT)."""
        mag = [0.0, 0.0, 0.0]
        # conversion factor from milli-gauss to nanoTesla (1 mG = 100 nT) and scaling factor from the sensor
