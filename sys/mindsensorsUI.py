...
class mindsensorsUI():
    ...
    def refresh(self):
        ...
        try:
            ...
            print("Error: Cannot write to /dev/fb1. Check permissions.")
    ...
    def drawButton(self, x, y, width, height, prefix="btns_", text="OK", display=True, align="left", image=None):
        ...
        self.fillRoundRect(x, y, width, height, 5, fill=(180,180,180), display=False)
        self.drawAutoText(text, x + 10, y + (height // 2) - 10, size=16, fill=(0,0,0), display=display)
    ...
    def isTouched(self):
        ...
        if tx == 0 and ty == 0:
            return False
        # Mapping for PiStorms landscape (Rotation 3)
        self.x = ty
        self.y = 240 - tx
        return True
